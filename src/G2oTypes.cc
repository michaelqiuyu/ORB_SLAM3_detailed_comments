...
// 更新世界坐标系
// xc's todo: 后续查看这个函数是如何使用的，也就是详细查看本质图优化的过程：4自由度位姿图优化
void ImuCamPose::UpdateW(const double *pu)
{
    ...
    if(its>=5)
    {
        // 对于轴角方向为(0, 0, 1)，其对应的旋转矩阵的DR(0, 2)、DR(1, 2)、DR(2, 0)、DR(2, 1)均为0
        // 由于ur为0, 0, theta，因此dR必然满足(0,2)、(1, 2)、(2, 0)和(2, 1)为0，DR从单位阵开始更新，因此DR理论上一定满足(0,2)、(1, 2)、(2, 0)和(2, 1)为0
        // 这样做是为了避免数值精度产生的影响---这个影响应该非常小，后续可以做个试验验证
        DR(0,2) = 0.0;
        DR(0,2) = 0.0;
        DR(0,2) = 0.0;
        DR(2,1) = 0.0;
